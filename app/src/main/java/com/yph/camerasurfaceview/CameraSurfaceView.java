...
public class CameraSurfaceView extends SurfaceView implements SurfaceHolder.Callback, Camera.AutoFocusCallback, View.OnClickListener {
    ...
    private void init(Context context) {
        ...
        Log.w("MetricsSize", dMetrics.heightPixels + "," + dMetrics.widthPixels +"; " + (float)dMetrics.heightPixels/ dMetrics.widthPixels);
...
//            Log.d("previewCallback", camera.getParameters().getPictureSize().width +" " + camera.getParameters().getPictureSize().height +"");
...
//            mCamera.addCallbackBuffer(previewBuffer);
    ...
    private void startPreview() {
        ...
        try {
            ...
            List<Size> supportList = mParam.getSupportedPreviewSizes();
            for(Size size : supportList){
                Log.d("SupportedPreviewSizes", "h * w = "+ size.height + " x " + size.width);
            }




            Size previewSize = CamParaUtil.getSize(mParam.getSupportedPreviewSizes(), 1080,
                    mCamera.new Size(VIDEO_1080[0], VIDEO_1080[1]));
            Toast.makeText(context, "previewSize:" + previewSize.width + " x " + previewSize.height, Toast.LENGTH_SHORT).show();
...
//            previewBuffer = new byte[yuv_buffersize];
            Size pictureSize = CamParaUtil.getSize(mParam.getSupportedPictureSizes(), 1080,
                    mCamera.new Size(VIDEO_1080[0], VIDEO_1080[1]));
            ...
            Toast.makeText(context, "screenOritation:" + screenOritation, Toast.LENGTH_SHORT).show();
            if (screenOritation != Configuration.ORIENTATION_LANDSCAPE) {
                ...
                mCamera.setDisplayOrientation(0);     // 竖着
//                mCamera.setDisplayOrientation(180);   // 倒着
            }
            if (mRunInBackground) {
...
//                mCamera.setPreviewCallbackWithBuffer(previewCallback);//设置摄像头预览帧回调
    ...
    public void switchLight(boolean open) {
        ...
        intent.setClassName("org.codeaurora.gallery", "com.android.gallery3d.app.GalleryActivity");
...
//        }
     ...
     **/
    public void capture() {
        Toast.makeText(context, "正在对焦,前后动一动...", Toast.LENGTH_SHORT).show();
        if (mCamera == null) return;
            ...
            try {
                mCamera.takePicture(null, null, new Camera.PictureCallback() {
                    ...
                    public void onPictureTaken(byte[] data, Camera camera) {
                        ...
                        Toast.makeText(context, "拍照成功:" + path + " [" + saved.length() / 1024 + " KB]", Toast.LENGTH_SHORT).show();
                        startPreview();


                        Intent mediaScan = new Intent(Intent.ACTION_MEDIA_SCANNER_SCAN_FILE, Uri.fromFile(saved));
    ...
    public boolean startRecord(int maxDurationMs, MediaRecorder.OnInfoListener onInfoListener) {
        ...
        Size videoSize = CamParaUtil.getSize(mParam.getSupportedVideoSizes(), 1080,
                mCamera.new Size(VIDEO_1080[0], VIDEO_1080[1]));
...
//        mediaRecorder.setPreviewDisplay(mSurfaceHolder.getSurface());//设置录制预览surface
        ...
        videoSavePath = FileUtil.getMediaOutputPath();
    ...
    public void stopRecord() {
        ...
        try {
            ...
            Intent mediaScan = new Intent(Intent.ACTION_MEDIA_SCANNER_SCAN_FILE, Uri.fromFile(new File(videoSavePath)));
            context.sendBroadcast(mediaScan);

            Toast.makeText(context, "视频已保存在图库", Toast.LENGTH_SHORT).show();
